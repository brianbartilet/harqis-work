...
def inspect_manifesto_compliance(batch: InspectionBatch):
    ...
    try:
        result = subprocess.run(
            [sys.executable, SCRIPTS_DIR / "manifesto_audit.py"],
            capture_output=True,
            text=True,
            timeout=10,
            cwd=REPO_ROOT
        ...
        if result.returncode != 0:
            batch.add(
                ...
                "scripts/manifesto_audit.py",
                "Fix missing manifesto blocks in workflow entries",
                f"Run: python scripts/manifesto_audit.py"
            )
            ...
            for line in lines:
                if "hard violation(s)" in line:
                    batch.add(
                        ...
                        f"Run: python scripts/manifesto_audit.py to see violations"
                    )
                    break
        ...
        batch.add(
            ...
            "Run manually: python scripts/manifesto_audit.py"
        )
