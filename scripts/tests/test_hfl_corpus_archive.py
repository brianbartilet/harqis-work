...
def test_document_date_uses_hfl_header_or_weekly_summary_title_without_mtime(tmp_path):
    hfl = _write(tmp_path / "daily.md", "## 2025-11-09 18:40\nMoment: Test\n")
    summary = _write(
        tmp_path / "_summary-2026-W22.md",
        "# HFL weekly summary — 2026-W22\nWindow: last 7 days · 2026-05-24 → 2026-05-30\n",
    )

    assert document_date(hfl) == datetime(2025, 11, 9).date()
    assert document_date(summary) == datetime(2026, 5, 25).date()
