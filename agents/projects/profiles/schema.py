...
class ProviderCredentialsConfig:
    """Per-profile Trello credentials (Mode A).

    Each field names the *env var* to read at runtime — the orchestrator looks
    them up and, when present, builds a per-profile `TrelloClient` so this
    agent's actions are attributed to its own Trello account.

    When all referenced env vars are unset, the orchestrator falls back to the
    shared client (Mode B — signed comments under the bot account).
    """
...
class DiscordIntegration:
    """Per-profile Discord posting allowlist + channel hints.

    The Discord bot itself is configured at the workspace level (env vars
    `DISCORD_BOT_TOKEN`, `DISCORD_GUILD_ID`). This block scopes which channels
    *this* agent is allowed to post to, and gives the agent a hint of what
    each channel is for so it can pick a good destination by inference.

    Empty `allowed_channels` disables the `discord_post` tool entirely for
    this profile, even if the bot is configured.
    """
    allowed_channels: list[str] = field(default_factory=list)
    channel_hints: dict[str, str] = field(default_factory=dict)


@dataclass
class IntegrationsConfig:
    """Per-profile integrations: external systems agents can post into.

    All integrations are opt-in — a profile that doesn't declare them gets
    no extra tools and no extra surface area.
    """
    discord: DiscordIntegration = field(default_factory=DiscordIntegration)
...
class AgentProfile:
    ...
    def from_dict(cls, data: dict[str, Any]) -> AgentProfile:
        return cls(
            ...
            integrations=_load_integrations(data.get("integrations", {})),
        )
    ...
    def merge_base(self, base: AgentProfile) -> AgentProfile:
        ...
        return AgentProfile(
            ...
            integrations=(
                self.integrations
                if self.integrations != IntegrationsConfig()
                else base.integrations
            ),
        )
...
def _load_integrations(d: dict) -> IntegrationsConfig:
    discord_d = d.get("discord", {}) or {}
    return IntegrationsConfig(
        discord=DiscordIntegration(
            allowed_channels=list(discord_d.get("allowed_channels", []) or []),
            channel_hints=dict(discord_d.get("channel_hints", {}) or {}),
        ),
    )
