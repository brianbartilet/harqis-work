"""
Tests for agents/projects/orchestrator/blocked_handler.py

TrelloClient is mocked — no API calls.
"""
...
def handler(mock_client):
    return BlockedCardHandler(client=mock_client, board_id="board123")
...
@pytest.mark.smoke
def test_poll_once_empty_blocked_column(handler, mock_client):
...
@pytest.mark.smoke
def test_poll_once_requeues_card_with_no_deps(handler, mock_client):
    card = _blocked_card()
    mock_client.get_cards.return_value = [card]
    n = handler.poll_once()
    assert_that(n, equal_to(1))
    mock_client.move_card.assert_called_once_with(card.id, "Ready")


@pytest.mark.smoke
def test_poll_once_posts_comment_when_requeuing(handler, mock_client):
    card = _blocked_card()
    mock_client.get_cards.return_value = [card]
    handler.poll_once()
...
@pytest.mark.smoke
def test_poll_once_leaves_card_blocked_when_secret_missing(handler, mock_client):
...
@pytest.mark.smoke
def test_poll_once_requeues_when_secret_now_present(handler, mock_client):
...
@pytest.mark.smoke
def test_poll_once_mixed_cards(handler, mock_client):
    resolved_card = _blocked_card("c1")
    still_blocked = _blocked_card("c2", custom_fields={"required_secrets": "MISSING_FOREVER_XYZ"})
    mock_client.get_cards.return_value = [resolved_card, still_blocked]
    env = {k: v for k, v in os.environ.items() if k != "MISSING_FOREVER_XYZ"}
    with patch.dict(os.environ, env, clear=True):
        n = handler.poll_once()
    assert_that(n, equal_to(1))
    move_calls = [c.args for c in mock_client.move_card.call_args_list]
    assert ("c1", "Ready") in move_calls
...
@pytest.mark.smoke
def test_poll_once_handles_provider_error_gracefully(handler, mock_client):
    mock_client.get_cards.side_effect = RuntimeError("network error")
    n = handler.poll_once()
    assert_that(n, equal_to(0))


@pytest.mark.smoke
def test_poll_once_continues_after_single_card_error(handler, mock_client):
    good_card = _blocked_card("good")
    mock_client.get_cards.return_value = [good_card]
    mock_client.add_comment.side_effect = [RuntimeError("comment failed")]
