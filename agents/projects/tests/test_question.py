...
Covers:
  ...
  - BoardOrchestrator.poll_resumes wiring (offline, mocked client)
"""
...
from agents.projects.agent.tools.kanban_tools import AskHumanTool
...
def _make_orchestrator(client: MagicMock, profile_id: str = "agent:default"):
    """Return a BoardOrchestrator wired to the given mock client, with a
...
def test_poll_resumes_skips_cards_without_question_label(sample_card):
    ...
    client.get_cards.return_value = [sample_card]

    orch, _ = _make_orchestrator(client)
    n = orch.poll_resumes()
    assert n == 0
...
def test_poll_resumes_dispatches_when_human_replied(sample_card):
    ...
    orch, _profile = _make_orchestrator(client)
