...
description: >
  ...
  you can't fulfil. SCAN phase writes a per-card CSV review list to harqis-work/results/
  (with an `approved` column defaulting to no, sorted by listing price desc); the
  operator edits `approved` to yes on the rows to action; the APPLY phase reads that
  approved CSV and marks sold in EchoMTG earnings + removes inventory + delists on
  TCG MP — only for approved rows. Wraps
  workflows.purchases.tasks.sold_inventory_radar (radar_sold_inventory + apply_radar_approvals).
  ...
  listings", "remove sold cards from inventory", "apply approved sold radar".
---
