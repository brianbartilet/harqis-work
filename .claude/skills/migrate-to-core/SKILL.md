...
description: >
  Sweep harqis-work for generic-automation code that belongs upstream in the
  harqis-core package (the `core` library, github.com/brianbartilet/harqis-core),
  and open a REVIEW-GATED pair of pull requests per candidate — one in harqis-core
  (adds the util + a unit test, ready) and one in harqis-work (switches to the
  upstream import, draft). Built to run headless on a monthly schedule. Ingests
  harqis-core first so it never proposes a duplicate; ignores workflows/ (service/
  chaining-specific) and the AI/Claude scaffold (harqis-core deliberately keeps AI
  in harqis-work). Nothing is ever auto-merged.
  Trigger phrases: "migrate to core", "harvest to core", "sweep harqis-work for
  core utilities", "monthly core migration", "what can move to harqis-core".
---
...
> Its README states AI/Claude was **deliberately moved out** into harqis-work, so
...
- Anything HFL/Kanban/manifesto/MCP/workflow-specific, or the AI/Claude scaffold.
...
3. **Respect the exclusions.** Never harvest `workflows/` (service/chaining), the
   AI/Claude scaffold (`apps/antropic` — core moved AI out on purpose), or anything
