...
description: >
  Generate BDD Gherkin (.feature) test scenarios from a specification — typically a Jira
  ticket URL/key, but also plain-text requirements or a Trello card description. Parses
  the Acceptance Criteria, asks targeted clarifying questions for any ambiguity, then
  produces idiomatic Gherkin with @functional / @integration / @negative / @edge tag
  coverage and an AC↔scenarios mapping table.

  Trigger phrases (non-exhaustive): "generate gherkin", "gherkin scenarios", "bdd
  scenarios for <ticket>", "write feature file for <key>", "AC tests for <ticket>",
  "cucumber tests for <ticket>".
