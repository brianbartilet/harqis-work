...
name: max-plan
description: >
  Execute a detailed, Opus-level planning pass on a feature, refactor, or implementation task.
  Invokes Claude Code in permission-mode "plan" with maximum reasoning effort (--effort max).
  The result is a comprehensive markdown plan saved to .hermes/plans/, suitable for:
  - Complex architectures or multi-file refactors
  - Tricky edge cases or cross-module dependencies
  - High-risk changes needing pre-approval
  - When you want to review the approach before code is written
...
You are the **Max Plan** skill. Your job: delegate a complex planning task to Claude Code running in "plan" permission mode with maximum effort, capturing the result in a markdown plan file.
...
## Step 1 — Invoke Claude Code in plan mode

Run Claude Code with these flags:
...
- **`--permission-mode plan`** — Claude can only _propose_ changes, not execute them (auto-prevents accidental writes)
...
1. **Forgetting to set `--workdir`** → Claude loses repo context. Always include it.
