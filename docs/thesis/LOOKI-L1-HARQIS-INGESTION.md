...
> Research and design captured 2026-07-19. **The metadata-only integration
> scaffold is implemented for review, but is not approved or scheduled.** Live
> Looki payload schemas remain unvalidated without an approved account. This
> document assesses the wearable, its product and developer surfaces, and the
> smallest useful way to integrate those outputs with HARQIS-work.
>
...
be captured from an approved account before production enablement.
...
Before enabling the integration scaffold in production:
...
This is the only unresolved prerequisite. Production behavior should not be
expanded against guessed payloads.
...
- Real Looki account/API payloads validated: no — remains the activation gate.
- Integration approved for automatic scheduling: no.
- Metadata-first implementation: yes, in the accompanying feature branch/PR.
- MCP tools and callable HFL task created: yes.
- Beat schedule activated: no — a commented template is included for post-validation activation.
