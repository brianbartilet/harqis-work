...
| No automated way to enforce manifesto alignment or surface drift | Principle 7 ("sharpen the saw") requires a check, not a vibe | Add `scripts/agents/repo-quality/manifesto_audit.py` — walks all `tasks_config.py`, validates the metadata, exits non-zero on violations |
...
### 4.3 `scripts/agents/repo-quality/manifesto_audit.py`
...
- **Review.** This PR description itself; the `scripts/agents/repo-quality/manifesto_audit.py` exit code; the operator's read of the per-workflow alignment tables. If any of the three surface drift, the fix is to update either the metadata or the manifesto, whichever is wrong.
...
| Metadata drifts from reality over time | Medium — same risk as any inline doc | `scripts/agents/repo-quality/manifesto_audit.py` is the safety net; consider wiring it into pre-commit later |
...
- [ ] `python scripts/agents/repo-quality/manifesto_audit.py` exits 0.
