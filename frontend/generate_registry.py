#!/usr/bin/env python3
"""
Generate frontend/registry.json from workflows/*/tasks_config.py.

For each workflow, the generator:
  - Pulls task_path, queue, and kwargs from tasks_config.py   (always authoritative)
  - Preserves label, description, schedule, manual_only from  the existing registry.json
...
def _load_existing_registry() -> dict:
    """Read frontend/registry.json (or {} if missing/invalid)."""
    path = FRONTEND_DIR / "registry.json"
    if not path.exists():
        return {}
    try:
        ...
        print(f"  Warning: could not load existing registry.json ({exc}) — starting fresh.")
        return {}
...
def main() -> None:
    out_path = FRONTEND_DIR / "registry.json"

    print("Generating frontend/registry.json from tasks_config.py files...")
    registry = build_registry()

    total = sum(len(wf["tasks"]) for wf in registry.values())
    print(f"  Workflows: {len(registry)}  |  Tasks: {total}")

    source   = json.dumps(registry, indent=2, ensure_ascii=False) + "\n"
