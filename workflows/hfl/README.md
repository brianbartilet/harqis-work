...
| `ingest_radar_activity` | Daily **DAILY RADAR digest.** Reads the day's `show_daily_radar` HUD briefings back out of the shared desktop feed file (`<feed_dir>/hud-logs-YYYYMMDD.txt`, the `@feed()` sink the radar writes to) and distils them into ONE "what the day was about" entry — it does **not** re-run the radar (which pulls ~9 sources + Sonnet every few hours), it ingests the briefings already produced. The feed file is Drive-synced, so the Beat host reads briefings a Windows radar wrote. The feed file(s) read are the entry's `references`. Haiku-distilled, raw fallback. No feed dir on this host / no radar briefings → no entry, no call. | `file:hfl_corpus+es:hfl-entries` | Daily 23:20 local (active — clean no-op until `DESKTOP_PATH_FEED` is set and the radar has run). |
...
### `ingest_radar_activity` (daily DAILY RADAR digest)

**Shipped active** in `tasks_config.py` — a clean no-op until there's signal.
The DAILY RADAR HUD (`workflows/hud/tasks/hud_radar.py :: show_daily_radar`)
