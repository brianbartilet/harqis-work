...
name: zapier-mcp
description: >
  Search, enable, and wire Zapier MCP actions into harqis-work workflows or use them
...
Before doing anything else, check whether the Zapier MCP server is connected in the current agent session.
...
4. In the active agent client, run: /update-config
   Add the Zapier MCP server URL when prompted.

   Or add it manually to the active agent client settings:
...
5. Restart this agent session so the tools load.
...
If `--workflow <name>` was passed, scaffold a workflow task that calls the Zapier action via the **Zapier REST API** so it can be scheduled or triggered by Celery Beat without an active agent session.
