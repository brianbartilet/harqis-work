---
name: create-new-fork-repository
description: >
  Fork the harqis-work repository into a fresh business- or client-scoped baseline that
  preserves the platform skeleton while stripping local-only tooling and workflows.
user-invocable: true
...
antropic        # Anthropic API (folder name has the historical typo;
...
```
.claude/                  # local agent-client config — fork team installs their own
...
If you have access to an agent client with the harqis-work skills installed, you can
...
must happen at the user's terminal, not from inside the agent client). Print:
...
Next steps for the host operator:
  ...
  [ ] Run `/deploy-harqis host` (after installing an agent client + skills in the fork) or
...
- Do NOT add agent-client config (`.claude/`) or AI agent state (`.hermes/`, legacy `.openclaw/`) to the fork — the consuming team installs its own.
