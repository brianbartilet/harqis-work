...
name: create-new-mcp
description: >
  Build or extend the MCP tool surface for an **existing** app under `apps/*`, wrapping
  its
user-invocable: true
...
def register_<app_name>_tools(mcp: FastMCP):
    ...
    def <verb>_<app_name>_<resource>(<args>) -> <list[dict] | dict>:
        """<One-line description the active agent reads to decide when to call this.>
...
Next steps:
  ...
  [ ] Reconnect in the active agent client: /mcp → harqis-work → Reconnect  (tool list is cached at connect)
