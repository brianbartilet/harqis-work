...
This skill **auto-invokes** whenever the user (via a Trello card, agent prompt, or
...
1. **Which component is affected?** Identify the file(s) most likely to change:
   ...
   - `.agents/skills/` for a skill change
...
- **A2.** Who or what triggers it — a scheduled task, a user action on the Kanban board,
  an MCP tool call from the active agent, a direct API call from another service?
