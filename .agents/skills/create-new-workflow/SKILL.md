...
description: >
  Design and implement an RPA-style Celery workflow that chains harqis-work app
  integrations and passes data between services.
user-invocable: true
...
**For AI/agent steps:** load the prompt from `workflows/<category>/prompts/` (see Step 6). Use `apps.antropic.references.web.base_api_service.BaseApiServiceAnthropic` for the API call. Follow the pattern in `workflows/finance/tasks/parse_transaction.py`.
...
## Step 6 - Generate AI prompts (if an agent / Anthropic is used)
...
Agent/Anthropic - there is no "if applicable" escape hatch. (A short,
