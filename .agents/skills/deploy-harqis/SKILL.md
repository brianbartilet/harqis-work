...
name: deploy-harqis
description: >
  Deploy the harqis-work platform on this machine as a full host or worker node,
  covering Docker services, env loading, workers, frontend, MCP, and orchestration.
user-invocable: true
...
| `--dry-run` | No | Run Kanban orchestrator in dry-run mode (logs actions, doesn't invoke the agent client). |
...
7b. **MCP server daemon** — runs `python scripts/launch.py mcp`. Note: typically the MCP server is spawned as a stdio subprocess by the active agent client, so this daemon is only needed for SSH-stdio remote access or HTTP-transport setups. Skip if `--no-mcp`.
